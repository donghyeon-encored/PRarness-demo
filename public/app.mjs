...
function renderIngredients(element, ingredients) {
  element.replaceChildren(...ingredients.map((ingredient) => {
...
async function scaleRecipe() {
  ...
  try {
    ...
    renderIngredients(ingredientsElement, result.ingredients);
    renderIngredients(shoppingListElement, result.shoppingList);
    shoppingListPanel.hidden = false;
...
renderIngredients(ingredientsElement, recipe.ingredients);
